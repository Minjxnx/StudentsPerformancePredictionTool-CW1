...
namespace StudentsPerformancePredictionTool_CW1.Forms
{
    public partial class ReportForm : Form
    {
        ...
        ReportService.ReportSoapClient reportService;
        public ReportForm(User user)
        {
            InitializeComponent();
            this.user = user;
            reportService = new ReportService.ReportSoapClient();
        ...
        private void GenerateReport(DateTime startDate, DateTime endDate)
        {
            ...
            // Call the web service to get the report data
            var reportData = reportService.GenerateReport(user.Name, startDate, endDate);

            // Populate the DataGridView with data
            foreach (var record in reportData)
            {
                dataGridViewReport.Rows.Add(record.Date.ToShortDateString(), record.StudyHours, record.StudySessions, record.BreakHours, record.BreakSessions);
            }
        }
